...
export class TodoEffects {
     ...
     * and for it, we pass only todoId, todoState, and todoDoneDate :)
    ...
    toggleCompleteTodo$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(action => this.todoService.update(action.todo)
            .pipe(
                map(() => toggleCompleteSuccessAction(
                    { todoId: action.todo.id, todoState: action.todo.state, todoDoneDate: action.todo.doneDate })),
                catchError((error: Error) => of(todoErrorAction({ error })))
            )
        ))
    );
    ...
    deleteTodo$ = createEffect(() => this.actions$.pipe(
        ...
        switchMap(action => this.todoService.delete(action.todoId)
            .pipe(
                map(() => deleteTodoSuccessAction({ tabIndex: action.tabIndex })),
                catchError((error: Error) => of(todoErrorAction({ error })))
            )
        ))
    );
